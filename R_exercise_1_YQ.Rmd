---
title: "R_exercise_1_YQ"
output: pdf_document
date: "2023-06-10"
---
#############################
# CHANGE DESCRIPTION
# I tabulate marriage status and urban status in the wages dataset.Using ggplot to look at the distribution of IQ and wages in a scatter plot. A positive relationship was found between IQ and wages. I fit another simple linear model where y is wage and x is IQ, and print out coefficients and other statistics. I construct a multiple linear model to control for urban status to draw association between IQ and wage. I found the coefficient of IQ becomes slightly smaller compared with that in the simple linear model. In the lfp dataset, I construct a logistic regression model with inlf as y and x are age and city, and print out results.
...
###Code change###
# Tabulate marriage status
table(wages$married)
# Tabulate urban status
table(wages$urban)
####
```
```{r}
...
### Code change##
ggplot(data=wages, aes(x=IQ, y=wage)) + geom_point() + stat_smooth(formula=y~x)
# Look at the distribution of IQ and wages in a scatter plot
###
```
...
### Code change###
model.results2 <- lm(wage ~ IQ, data=wages)
# Fit a linear model where y is wage and x is IQ
...
#####Code change
model.results.detail <- lm(wage ~ urban + IQ, data=wages)
summary(model.results.detail)
## Control for urban status to draw association between IQ and wage.
## The coefficient becomes slightly smaller.
...
####Code change
logit.model2 <- glm(inlf ~ age+city, data = lfp, family='binomial')
# Fit a logistic model using the 'glm' command with different variables.
